...
final class ReputationService
{
    ...
    public function getDealFeedbackStatus(Deal $deal, User $user, ?string $roleParam = null): array
    {
        ...
        return [
            ...
            'can_submit_feedback' => $deal->stage === DealStage::Completed && ! $hasSubmitted,
    ...
    private function resolveProfileEvidence(User $user, ParticipantRole $role): array
    {
        if ($role === ParticipantRole::Founder) {
            ...
            return [
                ...
                'businesses' => $businesses->map(fn (\App\Models\Business $b) => [
                    'id' => $b->id,
                    'name' => $b->name,
                    'industry' => $b->industry,
    ...
    private function enforceDealParticipant(Deal $deal, User $user, ?string $roleParam = null): void
    {
        ...
        if (! $isFounder && ! $isCounterparty) {
            abort(403, 'You are not an authorized participant in this Deal.');
        }

        // Multi-role explicit resolution check if role parameter is provided
        if ($roleParam !== null && trim($roleParam) !== '') {
            $requestedRole = ParticipantRole::tryFrom(trim($roleParam));
            if ($requestedRole === null) {
                throw ValidationException::withMessages([
                    'role' => ['Invalid role parameter provided.'],
                ]);
            }

            if ($isFounder && $requestedRole !== ParticipantRole::Founder) {
                abort(403, 'You are participating in this Deal as a founder, not '.$requestedRole->value.'.');
            }

            if ($isCounterparty && $requestedRole !== $deal->counterparty_role) {
                abort(403, 'You are participating in this Deal as a '.$deal->counterparty_role->value.', not '.$requestedRole->value.'.');
            }
        }
    }
}
