...
use App\Services\Deal\DealAgreementService;
use App\Services\Deal\DealNegotiationService;
...
class DealController extends Controller
{
    ...
    public function getNegotiation(
        Request $request,
        string $deal,
        DealNegotiationService $negotiationService
    ): JsonResponse {
        $dealModel = Deal::findOrFail($deal);
        $user = $request->user();
        $roleParam = $request->input('role') ?? $request->query('role');
    ...
    public function respondNegotiation(
        ...
        $dealModel = Deal::findOrFail($deal);
        $proposalModel = DealTermProposal::findOrFail($proposal);
        $user = $request->user();
    ...
    public function getAgreement(
        Request $request,
        string $deal,
        DealAgreementService $agreementService
    ): JsonResponse {
        $dealModel = Deal::findOrFail($deal);
        $user = $request->user();
        $roleParam = $request->input('role') ?? $request->query('role');
     ...
     */
    private function formatProposalResponse(DealTermProposal $proposal): array
    {
        return [
            'id' => $proposal->id,
     ...
     */
    private function formatAgreementResponse(DealAgreement $agreement): array
    {
        return [
            'id' => $agreement->id,
