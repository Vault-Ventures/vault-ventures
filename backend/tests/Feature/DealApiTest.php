...
class DealApiTest extends TestCase
{
    ...
    public function test_05_same_connection_and_same_parties_can_create_multiple_independent_deals(): void
    {
        $founder = $this->createFounder();
        $investor = $this->createInvestor();
        $business = $this->createBusiness($founder);
        ...
        // Second creation from the same connection
        $res2 = $this->actingAs($founder)->postJson("/api/me/connections/{$connection->id}/deal");
        $res2->assertCreated();
        $dealId2 = $res2->json('data.id');

        // They must be two distinct independent deals
        $this->assertNotEquals($dealId1, $dealId2);
        $this->assertDatabaseCount('deals', 2);
        $this->assertDatabaseCount('deal_state_histories', 2);

        // Advancing deal #1 does not affect deal #2
        $this->createMutualInterests($business, $founder, $investor, ParticipantRole::Investor);
        $this->actingAs($founder)->postJson("/api/me/deals/{$dealId1}/transition", [
            'target_state' => 'interest_confirmed',
        ])->assertOk()->assertJsonPath('data.stage', 'interest_confirmed');

        $deal2Check = $this->actingAs($founder)->getJson("/api/me/deals/{$dealId2}");
        $deal2Check->assertOk()->assertJsonPath('data.stage', 'matched');
    }
