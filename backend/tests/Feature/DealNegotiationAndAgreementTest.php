...
class DealNegotiationAndAgreementTest extends TestCase
{
    ...
    public function test_multi_deal_independence_proposals_and_agreements(): void
    {
        $founder = $this->createFounder();
        $investor = $this->createInvestor();
        $investorB = $this->createInvestor('investor_b@example.com');
        $business = $this->createBusiness($founder);
        $connectionA = $this->createConnection($business, $founder, $investor);
        $connectionB = $this->createConnection($business, $founder, $investorB);

        // Deal A
        $dealA = $this->createDeal($connectionA, DealStage::Negotiation);
        // Deal B
        $dealB = $this->createDeal($connectionB, DealStage::Negotiation);
        ...
        $getB = $this->actingAs($investorB)->getJson("/api/me/deals/{$dealB->id}/negotiation");
        $getB->assertOk()
    ...
    public function test_cross_deal_idor_protection(): void
    {
        $founder = $this->createFounder();
        $investor = $this->createInvestor();
        $investorB = $this->createInvestor('investor_b_idor@example.com');
        $business = $this->createBusiness($founder);
        $connectionA = $this->createConnection($business, $founder, $investor);
        $connectionB = $this->createConnection($business, $founder, $investorB);
        ...
        $resp = $this->actingAs($investorB)->postJson("/api/me/deals/{$dealB->id}/negotiation/{$propA->id}/respond", [
            'action' => 'accept',
        ]);
